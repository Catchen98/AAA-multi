...
from mot_neural_solver.tracker.mpn_tracker import VIDEO_COLUMNS, TRACKING_OUT_COLS
...
class MPNTracker:
    ...
    def __init__(
        self, graph_model, eval_params=None, dataset_params=None,
    ):
    ...
    def _estimate_frames_per_graph(self, det_df):
        """
        Determines the number of frames to be included in each batch of frames evaluated within a sequence
        """
        num_frames = len(det_df.frame.unique())
        num_detects = det_df.shape[0]

        avg_detects_per_frame = num_detects / float(num_frames)
        expected_frames_per_graph = round(
            self.dataset_params["max_detects"] / avg_detects_per_frame
        )

        return min(expected_frames_per_graph, self.dataset_params["frames_per_graph"])
    ...
    def _evaluate_graph_in_batches(self):
        ...
        frame_num_per_node = torch.from_numpy(self.full_graph.graph_df.frame.values).to(
            device
        )
        ...
        for eval_round, (start_frame, end_frame) in enumerate(
            zip(all_frames, all_frames[self.full_graph.frames_per_graph - 1 :])
        ):
            assert (
            ...
            edges_mask = (
                nodes_mask[self.full_graph.graph_obj.edge_index[0]]
                & nodes_mask[self.full_graph.graph_obj.edge_index[1]]
            )
        ...
        to_undirected_graph(
            self.full_graph, attrs_to_update=("edge_preds", "edge_labels")
        )
    ...
    def _project_graph_model_output(self):
        ...
        if self.eval_params["rounding_method"] == "greedy":
            ...
            projector = ExactProjector(
                self.full_graph, solver_backend=self.eval_params["solver_backend"]
            )
    ...
    def _assign_ped_ids(self):
        ...
        graph_shape = (
            self.full_graph.graph_obj.num_nodes,
            self.full_graph.graph_obj.num_nodes,
        )
        ...
        assert (
            len(labels) == self.full_graph.graph_df.shape[0]
        ...
        self.final_projected_output = self.final_projected_output[
            VIDEO_COLUMNS + ["conf", "detection_id"]
        ].copy()

    def track(self, full_graph, det_df):
        """
        ...
        frames_per_graph = self._estimate_frames_per_graph(det_df)
        self.full_graph.frames_per_graph = frames_per_graph
        ...
        if self.eval_params["add_tracktor_detects"]:
            self.final_projected_output = self._add_tracktor_detects(det_df)

        postprocess = Postprocessor(
            self.final_projected_output.copy(),
            seq_info_dict=None,
        ...
        final_out = self.tracking_out[TRACKING_OUT_COLS].sort_values(
            by=["frame", "ped_id"]
        )

        return final_out

    def _add_tracktor_detects(self, det_df):
        def ensure_detects_can_be_used(start_end_per_ped_id):
            ...
            triu_ixs, tril_ixs = (
                np.triu_indices_from(comparisons),
        ...
        big_dets_df = det_df.copy()
        complete_df = self.final_projected_output.merge(
            big_dets_df[
                [
                    "detection_id",
                    "tracktor_id",
